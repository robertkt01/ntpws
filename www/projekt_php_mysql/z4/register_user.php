...
$query = "INSERT INTO korisnik (ime, prezime, email, drzava_id, grad, ulica, datum_rodjenja, lozinka, username, role, active)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $db->prepare($query);
$stmt->bind_param('sssissssssi', $ime, $prezime, $email, $drzava_id, $grad, $ulica, $datum_rodjenja, $lozinka, $username, $role, $active);

if ($stmt->execute()) {
    header('Location: http://localhost/projekt_php_mysql/z4?menu=7&message=success');
} else {
